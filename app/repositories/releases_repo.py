...
def convert_to_releases_model(data:Any)-> Dict[str,Dict[str,str]]:
    ...
    if not isinstance(data, list):
        raise ValueError(f"Invalid response format: expected a list of releases. Data: {data}")
    
    for item in data:
        try:
            if not isinstance(item, dict):
                raise ValueError(f"Invalid release data format: expected a dictionary")
            if not "tag_name" in item or not "tarball_url" in item or not "zipball_url" in item:
                raise ValueError(f"Invalid release data format: missing required fields")
            if not isinstance(item["tag_name"], str) or not isinstance(item["tarball_url"], str) or not isinstance(item["zipball_url"], str):
                raise ValueError(f"Invalid release data format: expected fields to be string")
        except Exception as e:
            warnings.warn(f"Warning: couldn't fetch a release in item: {item}, skipping. Message: {e}")
        else:
            res[item["tag_name"]] = {"zipball_url":item["zipball_url"],"tarball_url":item["tarball_url"]}
...
class AppReleases:
    ...
    def __new__(cls,app_id:str):
        if not app_id in cls._instances:
    ...
    def load_releases(self):
        ...
        releases_url = f"https://api.github.com/repos/{app_item.owner}/{app_item.repo}/releases"
        try:
            response = get_http_response(releases_url)
            response_data = response.json()
            self._releases = convert_to_releases_model(response_data)
        except Exception as e:
            print(f"Error: Failed to load releases of app with id {self._id}. Message: {e} ")
        else:
